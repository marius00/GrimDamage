...
function createChartPetDamageDealt(id, sequenceLength, colors, previousChart) {
    ...
    if (previousChart && previousChart.series) {
        ...
        const damageTypes = [
            'Physical', 'Lightning', 'Vitality', 'Aether', 'Bleeding', 'Poison', 'Pierce', 'Chaos',
            'PercentCurrentLife', 'LifeLeech', 'Cold', 'Fire'
        ];
        const turbo = 0;
        preloadSeries = [
            {
        ...
        for (let idx = 0; idx < damageTypes.length; idx++) {
            preloadSeries.push(
                {
                    name: damageTypes[idx],
                    marker: { enabled: false },
                    color: colors.color(damageTypes[idx]),
                    data: Array.from(Array(sequenceLength), () => null),
                    turboThreshold: turbo
                }
            );
    ...
    return Highcharts.chart(id,
        {
            ...
            tooltip: {
                pointFormat: 'Dealt <b>{point.y:,.0f}</b> {series.name} damage'
            },
            plotOptions: {
                series: {
                    turboThreshold: 0
